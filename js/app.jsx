...
// Global dosyalardan verileri çekiyoruz
...
const App = () => {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('carpenter_final_v25_cart');
        if (saved) setCart(JSON.parse(saved));
        const savedSettings = localStorage.getItem('carpenter_final_v25_settings');
        if (savedSettings) setSettings(JSON.parse(savedSettings));
    }, []);

    useEffect(() => {
        localStorage.setItem('carpenter_final_v25_cart', JSON.stringify(cart));
        localStorage.setItem('carpenter_final_v25_settings', JSON.stringify(settings));
    }, [cart, settings]);
    ...
    const addToCart = (product) => {
        ...
        const newItem = {
            ...
            qty: 1,
            config: {
                width: product.defaultWidth, width2: 90, width3: 100,
                ...
                hardwareBrand: bulkConfig.hardwareBrand, cabinetShape: ['corner_base', 'corner_upper'].includes(product.type) ? 'L' : 'Düz', 
                                        ...
                                        {cart.map(item => (
                                            <div key={item.id} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex flex-col gap-6 border-l-4 border-l-slate-100 hover:border-l-black transition-all group uppercase tracking-tighter">
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-6 lg:grid-cols-12 gap-2 text-[8px] font-black uppercase italic tracking-widest leading-none">
                                                    ...
                                                    {item.product.type === 'wall_cabinet' && item.config.cabinetShape === 'U' && (
                                                    ...
                                                    {item.product.type === 'wall_cabinet' && (
                                                        <>
                                                            <div className="col-span-2 uppercase italic"><label className="text-slate-300 block mb-1 italic">Kapak Türü</label><select value={item.config.doorType} onChange={e => updateItemConfig(item.id, 'doorType', e.target.value)} className="w-full bg-slate-50 p-2.5 rounded-xl font-black outline-none uppercase"><option value="hinged">Menteşe</option><option value="sliding">Sürme</option></select></div>
                                                            <div className="col-span-2 uppercase italic"><label className="text-slate-300 block mb-1 italic">Dolap Şekli</label><select value={item.config.cabinetShape} onChange={e => updateItemConfig(item.id, 'cabinetShape', e.target.value)} className="w-full bg-slate-50 p-2.5 rounded-xl font-black outline-none uppercase"><option value="Düz">Düz</option><option value="L">L Tipi</option><option value="U">U Tipi</option></select></div>
                                                        </>
                                                    )}
